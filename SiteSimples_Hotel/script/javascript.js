...
function validaCadastro(){
  ...
  if (email.indexOf("@") < 0) {
    ...
    document.getElementById("emailErro").innerHTML="⚠️ O email deve começar com uma letra";
    erro = true;
  }
  ...
  if (tam >= 6 && senha1.match(/^[a-zA-Z]+$/)) {
    if (senha1.match(/^[0-9]+/)) {
      document.getElementById("erroSenhaForca").innerHTML = "<span style='color: red'>Fraca</span>";
    }
  } else if (tam == 6) {
    document.getElementById("erroSenhaForca").innerHTML = "<span style='color: red'>Fraca</span>";
  } if (tam > 6 && senha1.match(/^((?=.*[a-zA-Z])(?=.*[0-9])(?=.*[|!"$%&@#\/\(\)\?\^\'\\\+\-\*]))^.*$/)) {
    document.getElementById("erroSenhaForca").innerHTML = "<span style='color: #00FF00'>Média</span>";
  } if (tam > 6 && senha1.match(/^((?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])(?=.*[|!"$%&@#\/\(\)\?\^\'\\\+\-\*]))^.*$/)) {
    document.getElementById("erroSenhaForca").innerHTML = "<span style='color: #0000FF'>Forte</span>";
  }
